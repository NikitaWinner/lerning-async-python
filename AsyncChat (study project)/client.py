...
from common.settings import ACTION, PRESENCE, TIME, USER, ACCOUNT_NAME, \
    RESPONSE, DEFAULT_PORT, ERROR, DEFAULT_IP_ADDRESS, MESSAGE_TEXT, MESSAGE, SENDER
...
@Log(CLIENT_LOGGER)
def create_message(sock, account_name='Guest') -> dict:
...
def main():
    ...
    server_address, server_port, client_mode = get_arg_commandline()
    ...
    try:
        ...
        # далее основной цикл прогрммы:
        print('Режим работы - отправка сообщений.' if client_mode == 'send' else 'Режим работы - приём сообщений.')
        # if client_mode == 'send':
